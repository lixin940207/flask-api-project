...
class RelationMarkJobListResource(Resource):
    ...
    def get(
        ...
        count, result = MarkJobService().get_mark_job_list_by_nlp_task(args, NlpTaskEnum.relation)
        return {
                   "message": "请求成功",
                   "result": result,
                   "count": count,
    ...
    def post(
        ...
        result = MarkJobService().create_mark_job(files, NlpTaskEnum.relation, args)
...
class RelationMarkJobItemResource(Resource):
...
class RelationMarkJobMultiDelete(Resource):
    @parse({
